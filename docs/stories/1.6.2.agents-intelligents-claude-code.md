...
- [ ] Créer les types TypeScript pour l'intégration Claude Code

### Phase 2: Assistant Claude Code Unifié (AC: 2)
- [ ] Créer service unifié `ClaudeCodeAssistant` dans `lib/services/`
- [ ] Implémenter interface simple pour les interactions avec Claude Code
- [ ] Configurer les outils Claude Code (Read, Grep, Glob, LS) avec permissions
- [ ] Développer système de gestion des sessions Claude Code
- [ ] Créer types TypeScript pour l'intégration Claude Code

### Phase 3: Investigation Autonome (AC: 3)
- [ ] Créer `lib/services/codebase-analyzer.ts` pour orchestration
- [ ] Implémenter investigation automatique avec outils Claude Code
- [ ] Développer cache intelligent pour les investigations
- [ ] Créer indicateurs visuels pour actions d'investigation
...
- [ ] Créer affichage transparent du processus de pensée
- [ ] Développer sélecteur automatique de format de réponse
- [ ] Ajouter indicateurs visuels pour états de raisonnement
...
- [ ] Créer `DocumentationGenerator` pour insertion auto via UniversalContentPanel
...
- [ ] Intégrer agents avec UniversalChatPanel (Story 1.6.1)
- [ ] Implémenter streaming des réponses en temps réel
- [ ] Créer endpoints API pour gestion des agents
- [ ] Tests unitaires et d'intégration complets

### Phase 7: Affichage Avancé du Chat (AC: 7)
- [ ] **Intégration d'UniversalContentPanel pour l'affichage de code**
- [ ] **Système de métadonnées Claude Code** (outils utilisés, fichiers analysés)
- [ ] **États d'investigation et de raisonnement** avec indicateurs visuels
- [ ] **Messages enrichis** avec support des différents types de contenu
- [ ] **Preview de code** avec coloration syntaxique Monaco Editor
- [ ] **Indicateurs d'actions Claude Code** (investigation, analyse, etc.)
- [ ] **Tests d'affichage** pour valider le rendu des différents types de contenu
...
src/lib/
├── services/
│   ├── claude-code-assistant.ts       # Service principal Claude Code unifié
│   ├── claude-session-manager.ts      # Gestion des sessions Claude Code
│   └── documentation-generator.ts     # Génération via UniversalContentPanel
├── types/
│   ├── claude-code.ts                 # Types pour Claude Code SDK
│   └── claude-session.ts              # Types pour sessions Claude Code
└── utils/
    └── claude-tools.ts                # Utilitaires pour outils Claude Code
```
...
4. **Raisonnement Multi-Étapes** : Transparence et processus
5. **Agents Spécialisés** : Analysis et Documentation
6. **Intégration** : UniversalChatPanel et tests

**Avantages de cette Approche :**
- Assistant Claude Code unifié et simple à utiliser
- Transparence totale du processus de pensée
...
- Réutilisabilité pour différentes pages de l'application
