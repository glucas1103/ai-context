...
export async function POST(
  ...
  try {
    ...
    if (name.length > 255) {
      return NextResponse.json({ error: 'Le nom du dossier ne peut pas dépasser 255 caractères' }, { status: 400 })
    }
    ...
    if (!nameRegex.test(name)) {
      return NextResponse.json({ 
        error: 'Le nom du dossier ne peut contenir que des lettres, chiffres, espaces, tirets, underscores et points' 
      }, { status: 400 })
    }
    ...
    return NextResponse.json({ data: newFolder }, { status: 201 })
  } catch (error) {
    console.error('Erreur dans l\'API folders:', error)
